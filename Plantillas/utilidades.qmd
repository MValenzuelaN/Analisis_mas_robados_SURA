...
-   **connection_string**: detecta automáticamente el driver ODBC disponible (18 o 17) y construye la cadena de conexión a `SQL Server` con autenticación de Windows.
...
def connection_string(
    server: str = "SGF1025",
    database: str = "Transaccional",
    trusted: bool = True,
    encrypt: bool = False,
) -> str:
    """Construye la cadena de conexión ODBC para SQL Server.

    Detecta automáticamente si el sistema dispone del ODBC Driver 18
    o el 17 y selecciona el más reciente. Lanza ``RuntimeError``
    cuando no se encuentra ninguno de los dos.

    Args:
        server: Nombre o IP del servidor SQL Server.
        database: Base de datos de destino.
        trusted: Si ``True``, usa autenticación integrada de Windows.
        encrypt: Si ``True``, habilita cifrado TLS en la conexión.

    Returns:
        Cadena de conexión lista para ``pyodbc.connect()``.

    Raises:
        RuntimeError: Si no se encuentra ODBC Driver 17 ni 18.
    """
    drivers_disponibles: list[str] = pyodbc.drivers()

    driver: str | None = None
    for candidato in ("ODBC Driver 18 for SQL Server",
                      "ODBC Driver 17 for SQL Server"):
        if candidato in drivers_disponibles:
            driver = candidato
            break

    if driver is None:
        raise RuntimeError(
            "No se encontró 'ODBC Driver 18 for SQL Server' ni "
            "'ODBC Driver 17 for SQL Server'. Drivers disponibles: "
            f"{drivers_disponibles}"
        )
...
print("[OK] Función connection_string definida (detecta Driver 18/17).")
```
...
```{python}
#| label: Actualizar tabla de datos SQLSERVER
#| echo: true
#| cache: true

# from datetime import datetime, timedelta

def parquet_expirado(parquet_path, diff_dias):
    if not parquet_path.exists():
        return True

    fecha_modificacion = datetime.fromtimestamp(parquet_path.stat().st_mtime)
    fecha_actual = datetime.now()

    diferencia = fecha_actual - fecha_modificacion

    return diferencia > timedelta(days=diff_dias)
```
