...
def ejecutar_qmd(nombre_archivo):
    ...
    PROJECT_ROOT = carpeta_script.parent

    if hay_archivos_en_excel(PROJECT_ROOT):
        print("[ERROR] Proceso detenido: existen archivos en la carpeta Excel.")
        return

    print("Buscando Informe_mas_robados.qmd...")

    if not ruta_archivo.exists():
        print(f"[ERROR] El archivo '{nombre_archivo}' no existe.")
        return

    print("Informe_mas_robados.qmd encontrado")

    try:
        resultado = subprocess.run(
            ["quarto", "render", str(ruta_archivo), "--output-dir", str(ruta_archivo.parent)],
            capture_output=True,
            text=True,
...
def mover_html(origen):
    ...
    destino_dir = PROJECT_ROOT / "Salidas" / "Informes_tecnicos_SURA"
...
if __name__ == "__main__":
    ejecutar_qmd("Informe_mas_robados.qmd")
