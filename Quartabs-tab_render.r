...
print_tabset_start <- function(heading_levels, i, tabset_master, tabset_div) {
    if (
        is.na(heading_levels[1]) &&
...
print_tabset_end <- function(heading_levels, i, len_tab, tabset_master) {
    lapply(rev(seq_len(len_tab)), function(j) {
        if (
            is.na(heading_levels[j]) &&
...
validate_data <- function(
    ...
    stopifnot(
        ...
        "`data` must have one or more rows." = nrow(data) >= 1,
        "`data` must have two or more columns." = ncol(data) >= 2
    )

    if (!is.null(layout)) {
        stopifnot(
            "`layout` must be length 1." = length(layout) == 1,
            "`layout` must be character." = is.character(layout),
            '`layout` must begin with at least three or more repetitions of ":".' = grepl(
                "^:{3,}",
                layout
            )
        )
    ...
    if (!is.null(heading_levels)) {
        stopifnot(
            "`heading_levels` must be numeric." = is.numeric(heading_levels),
            "`heading_levels` must be length 1 or greater." = length(
                heading_levels
            ) >
                0,
            "`heading_levels` must not include NaN." = !is.nan(heading_levels),
            "`heading_levels` must not be infinite." = !is.infinite(
                heading_levels
            )
        )
        ...
        if (length(nums) > 0) {
            stopifnot(
                "`heading_levels` except for NAs must be positive." = nums > 0
            )
        }
    ...
    stopifnot(
        "`tabset_vars` must be of length 1 or more." = len_tab > 0
    )
        ...
        function(nm) {
            ...
            if (!inherits(col, valid_class_for_tabset_name)) {
                stop(
                    ...
                    ".\n",
                    sprintf(
                        "But the column `%s` has the following class(es): ",
                        nm
                    ),
                    toString(class(col)),
                    "."
    ...
    and the length of `heading_levels` must be the same." = length(
            heading_levels
        ) ==
            len_tab
    )
    ...
    stopifnot(
        "`output_vars` must be of length 1 or more." = length(output_names) > 0,

        "There must not be variables that are included in both
    `tabset_vars` and `output_vars`." = length(intersect(
            tabset_names,
            output_names
        )) ==
            0
